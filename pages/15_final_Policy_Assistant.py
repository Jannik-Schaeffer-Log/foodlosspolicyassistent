...
with st.expander('Details Regression'):
    st.write('Datatable with Regression Coef. per Income Group:')
    st.dataframe(df_reg_income_total)
    chart_df_reg_income_total=df_reg_income_total
    chart_df_reg_income_total.index=['Tracking and Tracing', 'Service Quality', 'International shipments', 'Customs','Timeliness', 'Infrastructure']
    st.write(chart_df_reg_income_total)
    chart_df_reg_income_total=chart_df_reg_income_total.reset_index().melt(id_vars=['index'])

    dict_income_group_dt={
    'Upper-Middle-Income Economies':'Hohes-Mittleres Einkommen',
    'Low-Income Economies':'Niedriges Einkommen',
    'Lower-Middle Income Economies':'Niedriges-Mittleres Einkommen',
    'High-Income Economies':'Hohes Einkommen',
    ...
    chart_df_reg_income_total['variable']=chart_df_reg_income_total['variable'].replace(dict_income_group_dt)
    chart_df_reg_income_total['variable']=pd.Categorical(chart_df_reg_income_total['variable'],["Hohes Einkommen","Hohes-Mittleres Einkommen","Niedriges-Mittleres Einkommen","Niedriges Einkommen"])
    chart_df_reg_income_total=chart_df_reg_income_total.sort_values('variable')
    #df['m'] = pd.Categorical(df['m'], ["March", "April", "Dec"])

    fig_income_reg=px.bar(chart_df_reg_income_total, x='index', y='value', 
                 color="variable", barmode="group")
    fig_income_reg.update_layout(xaxis_title='LPI-Kategorie', yaxis_title='Regressionskoeffizient',title='Säulendiagramm des Regressionskoeffizienten',legend_title_text='Einkommensgruppe', template='seaborn')
    st.plotly_chart(fig_income_reg, use_container_width=True)
    st.write('country regression')
    st.dataframe(df_reg_country)
